...
export const FaiEditModal: React.FC<FaiEditModalProps> = ({
  isOpen, onClose, job, onSaveJob,
}) => {
  const [formData, setFormData] = useState<EngineeringJob | null>(null);

  useEffect(() => {
    if (job) {
      const currentStatus = (job.status === 'Draft' ? 'Edit' : job.status) as PipelineStatus;
      setFormData({
        ...job,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const allPassed = allChecks && formData.passedTest && formData.passedQa;

    let finalStatus: PipelineStatus = formData.status as PipelineStatus;
    ...
    onSaveJob({ ...formData, status: finalStatus });
    onClose();
  };

  const handleToggleCheck = (checkKey: keyof NonNullable<EngineeringJob['checks']>) => {
    setFormData((prev) => {
      if (!prev) return null;
      return {
        ...prev,
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4 text-xs max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-2 bg-slate-50 border border-slate-200 rounded font-semibold text-slate-800 focus:bg-white focus:ring-2 focus:ring-sky-500 cursor-pointer"
              >
          ...
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
            <span className="text-[11px] font-semibold text-slate-700 uppercase tracking-wide flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-sky-600" /> Build Timing Details
            </span>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
              ...
              <div>
                <label className="block font-medium text-slate-700 mb-1">Total Build Time (Hours)</label>
                <input
                  type="number"
                  step="0.1"
                  min="0"
                  value={formData.totalBuildTimeHours}
                  onChange={(e) => setFormData({ ...formData, totalBuildTimeHours: e.target.value ? parseFloat(e.target.value) : '' })}
          ...
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <span className="text-[11px] font-semibold text-slate-700 uppercase tracking-wide flex items-center gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Pre-Dispatch Quality Gates
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                <button type="button" onClick={() => handleSetAllChecks(true)} className="px-2 py-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded cursor-pointer">
                  Pass All Gates
                </button>
                <button type="button" onClick={() => handleSetAllChecks(false)} className="px-2 py-1 text-[10px] font-semibold text-slate-600 bg-white hover:bg-slate-100 border border-slate-200 rounded cursor-pointer">
                  Reset All
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
              <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer">
                <input type="checkbox" checked={formData.checks?.xyOdb} onChange={() => handleToggleCheck('xyOdb')} className="w-4 h-4 text-sky-600 rounded" />
                <span className="text-slate-700 font-medium">#1 XY/ODB++ Verified</span>
              </label>
              <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer">
                <input type="checkbox" checked={formData.checks?.stencilBotTop} onChange={() => handleToggleCheck('stencilBotTop')} className="w-4 h-4 text-sky-600 rounded" />
                <span className="text-slate-700 font-medium">#2 Stencil File BOT/TOP</span>
              </label>
              <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer">
                <input type="checkbox" checked={formData.checks?.spiBotTop} onChange={() => handleToggleCheck('spiBotTop')} className="w-4 h-4 text-sky-600 rounded" />
                <span className="text-slate-700 font-medium">#3 SPI File BOT/TOP</span>
              </label>
              <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer">
                <input type="checkbox" checked={formData.checks?.pnpBotTop} onChange={() => handleToggleCheck('pnpBotTop')} className="w-4 h-4 text-sky-600 rounded" />
                <span className="text-slate-700 font-medium">#4 PNP File BOT/TOP</span>
              </label>
              <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer">
                <input type="checkbox" checked={formData.checks?.aoiBotTop} onChange={() => handleToggleCheck('aoiBotTop')} className="w-4 h-4 text-sky-600 rounded" />
                <span className="text-slate-700 font-medium">#5 AOI File BOT/TOP</span>
              </label>
              <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 cursor-pointer">
                <input type="checkbox" checked={formData.checks?.aoiFinalBotTop} onChange={() => handleToggleCheck('aoiFinalBotTop')} className="w-4 h-4 text-sky-600 rounded" />
                <span className="text-slate-700 font-medium">#8 AOI Final File</span>
              </label>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              <div className="p-2.5 bg-white border border-slate-200 rounded-lg space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <select
                    value={formData.passedTest ? 'Yes' : 'No'}
                    onChange={(e) => setFormData({ ...formData, passedTest: e.target.value === 'Yes' })}
                    className={`text-[11px] font-bold px-2 py-1 rounded border cursor-pointer ${formData.passedTest ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-rose-100 text-rose-800 border-rose-300'}`}
                  >
                ...
                <div>
                  ...
                  <input type="date" value={formData.passedTestDate || ''} onChange={(e) => setFormData({ ...formData, passedTestDate: e.target.value })} className="w-full text-xs font-mono p-1.5 bg-slate-50 border border-slate-200 rounded" />
                </div>
              </div>
              ...
              <div className="p-2.5 bg-white border border-slate-200 rounded-lg space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <select
                    value={formData.passedQa ? 'Yes' : 'No'}
                    onChange={(e) => setFormData({ ...formData, passedQa: e.target.value === 'Yes' })}
                    className={`text-[11px] font-bold px-2 py-1 rounded border cursor-pointer ${formData.passedQa ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-rose-100 text-rose-800 border-rose-300'}`}
                  >
                    <option value="Yes">Yes</option>
                    <option value="No">No</option>
                  </select>
                ...
                <div>
                  ...
                  <input type="date" value={formData.passedQaDate || ''} onChange={(e) => setFormData({ ...formData, passedQaDate: e.target.value })} className="w-full text-xs font-mono p-1.5 bg-slate-50 border border-slate-200 rounded" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end pt-3 border-t border-slate-200 gap-2">
            ...
            <button type="submit" className="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-semibold text-white bg-sky-600 hover:bg-sky-500 rounded-lg shadow-sm cursor-pointer">
              <Save className="w-3.5 h-3.5" /> Save FAI Changes
            </button>
          </div>
        </form>
      </div>
